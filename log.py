...
#     target_depth = adv_depth.detach().cpu()[0]
...
#     target_depth = (adv_depth - adv_depth.min()) / (adv_depth.max() - adv_depth.min())

#     ben_depth = colormap(ben_depth.numpy())[..., :3]
#     adv_depth = colormap(adv_depth.numpy())[..., :3]
#     target_depth = colormap(adv_depth.numpy())[..., :3]
...
#     log.add_image(f'{name_prefix}/train/target_scene', ben_scene_image.detach().cpu()[0], epoch)
...
#     log.add_image(f'{name_prefix}/train/target_depth', adv_depth, epoch, dataformats='HWC')
...
def log_img_train(log, epoch, name_prefix, images):
    ...
    # CPU로 내리고 첫 번째 배치 이미지만 가져오기
    ben_depth = ben_depth.detach().cpu()[0]
    adv_depth = adv_depth.detach().cpu()[0]
    target_depth = target_depth.detach().cpu()[0] # [버그 수정] adv_depth가 아닌 target_depth 할당
    ...
    target_depth = (target_depth - target_depth.min()) / (target_depth.max() - target_depth.min() + 1e-7) # [버그 수정]

    # 컬러맵 씌우기 (RGB 3채널로 변환)
    ben_depth = colormap(ben_depth.numpy())[..., :3]
    adv_depth = colormap(adv_depth.numpy())[..., :3]
    target_depth = colormap(target_depth.numpy())[..., :3] # [버그 수정]
  
    # 텐서보드에 이미지 로깅
    log.add_image(f'{name_prefix}/train/adv_scene', adv_scene_image.detach().cpu()[0], epoch)
    log.add_image(f'{name_prefix}/train/ben_scene', ben_scene_image.detach().cpu()[0], epoch)
    log.add_image(f'{name_prefix}/train/target_scene', target_scene_image.detach().cpu()[0], epoch) # [버그 수정]
    log.add_image(f'{name_prefix}/train/patch', patch.detach().cpu()[0], epoch)
    
    log.add_image(f'{name_prefix}/train/ben_depth', ben_depth, epoch, dataformats='HWC')
    log.add_image(f'{name_prefix}/train/adv_depth', adv_depth, epoch, dataformats='HWC')
    log.add_image(f'{name_prefix}/train/target_depth', target_depth, epoch, dataformats='HWC') # [버그 수정]

# log.py 에 아래 함수를 추가 (기존 log_img_train 함수 밑에 붙여넣기)
